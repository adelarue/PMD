...
# for n in n_list
    ...
    Xmatrix = 1 .* (rand(size(Xmatrix,1), size(Xmatrix,2)) .< Xmatrix)
    ...
    for aux_num in 1:length(missingness_proba_list)
        ...
        for c in names(X_missing)
            if any(ismissing.(X_missing[:,c]))
                c_ind = c*"_Missing"
                X_missing[!, c_ind] .= 1 .* ismissing.(X_missing[:,c])
                X_missing[ismissing.(X_missing[:,c]), c] .= 0
            end
        end
        ...
        # for iter in 1:10
            ...
            if do_impthenreg
                for model in [:linear, :tree, :rf]
                    ...
                    ## Method 1.1: Encoding as a new category
                    ...
                    push!(results_table, [dname, SNR, k, missingness_proba, iter, "Encoding as new category - $(model)", R2, OSR2,  R2l, OSR2l, [], δt, bestparams, score])
                    # CSV.write(savedir*filename, results_table)
                    ...
                    R2, OSR2 = PHD.evaluate(Y, df, linear)
                    R2l, OSR2l = PHD.stratified_evaluate(Y, df, linear, patidx, subsetpattern=subsetpattern)   
                    push!(results_table, [dname, SNR, k, missingness_proba, iter, "Mode impute - $(model)", R2, OSR2, R2l, OSR2l, [], δt, bestparams, score])
                    CSV.write(savedir*filename, results_table)
